...
- [ ] Вынести вложения в отдельный слой `context providers`, чтобы не смешивать их с download pipeline видео.
- [ ] Нормализовать текстовые вложения в единый внутренний формат с типом, именем файла и размером.
- [ ] Для кодовых вложений сохранять язык/расширение, чтобы промпт мог корректно ссылаться на фрагменты.
- [ ] Для изображений считать budget по эвристике с запасом, а не делать вид, что offline token count точен.
