...
def test_export_txt_and_json(tmp_path: Path):
...
def test_export_srt_and_vtt_formats(tmp_path: Path):
    """SRT and VTT formats produce time-coded outputs."""
    doc = build_doc()
    srt = ex.export_srt(doc)
    assert "00:00:00,000" in srt and "-->" in srt
...
def test_export_srt_with_rules_splits_long_text():
...
def test_fill_empty_gaps_in_srt_adjusts_end_times():
...
def test_metadata_append_extract_and_strip():
    """ASK metadata helpers append, extract and strip correctly."""
    base = "1\n00:00:00,000 --> 00:00:01,000\nHello\n\n"
    with_meta = ex.append_ask_metadata_to_srt(base, tool_name="ASK", quality="fast", completed=True)
    meta = ex.extract_ask_metadata_from_srt(with_meta)
    assert meta and meta.get("tool") == "ASK" and meta.get("completed") is True
