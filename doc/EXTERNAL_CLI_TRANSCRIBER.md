...
3. Если истёк `--client-timeout`, клиент **сигнализирует демону дроп** задания (cancel-маркер) и выходит с кодом `10`, чтобы демон не тратил ресурсы на брошенную задачу и очередь не копила «висяки».
